...
std::string read_fixture(const char* name) {
    std::ifstream file(std::string(OFFICER_QUERY_FIXTURE_DIR) + "/" + name);
    return {std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>()};
}

void test_process_create() {
    std::string error;
    const auto event = panopticon::officer::query::SysmonParser::parse_xml(
        read_fixture("sysmon_process_create.xml"), error);
...
void test_network_file_and_registry() {
    std::string error;
    const auto network = panopticon::officer::query::SysmonParser::parse_xml(
        read_fixture("sysmon_network_connect.xml"), error);
    expect(network && network->network && network->network->destination_port == 443, "network fields parse");
    const auto file = panopticon::officer::query::SysmonParser::parse_xml(
        read_fixture("sysmon_file_create.xml"), error);
    expect(file && file->file && file->file->path.has_value(), "file fields parse");
    const auto registry = panopticon::officer::query::SysmonParser::parse_xml(
        read_fixture("sysmon_registry_value_set.xml"), error);
    ...
    const auto registry_create_delete = panopticon::officer::query::SysmonParser::parse_xml(registry_xml, error);
    ...
    const auto registry_rename = panopticon::officer::query::SysmonParser::parse_xml(registry_xml, error);
...
void test_missing_and_invalid_values() {
    ...
    const std::string command_line = "<Data Name=\"CommandLine\">&quot;C:\\OfficerLab\\sample.exe&quot; --safe-test</Data>\n    ";
    xml.erase(xml.find(command_line), command_line.size());
    std::string error;
    const auto missing = panopticon::officer::query::SysmonParser::parse_xml(xml, error);
    ...
    error.clear();
    expect(!panopticon::officer::query::SysmonParser::parse_xml(xml, error).has_value() &&
               error.find("ProcessId") != std::string::npos,
           "invalid numeric field fails explicitly");

    error.clear();
    expect(!panopticon::officer::query::SysmonParser::parse_xml("<Event>", error).has_value() &&
               !error.empty(),
           "malformed XML fails safely");
}
