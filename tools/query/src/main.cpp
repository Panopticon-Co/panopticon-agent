...
nlohmann::json to_json(const panopticon::officer::query::TelemetryEvent& event) {
    return {
        ...
        {"network", json_object(event.network, [](const panopticon::officer::query::NetworkDetails& network) {
        ...
        {"file", json_object(event.file, [](const panopticon::officer::query::FileDetails& file) {
            return nlohmann::json{{"path", json_value(file.path)}, {"creation_utc", json_value(file.creation_utc)}};
        })},
        {"registry", json_object(event.registry, [](const panopticon::officer::query::RegistryDetails& registry) {
...
int main(int argc, char* argv[]) {
    ...
    const auto xml_events = panopticon::officer::query::EventLogReader::read_newest_event_xmls(
        options->event_id, options->limit, error_message);
    if (!xml_events) {
        std::cerr << "Officer Query acquisition failed: " << error_message << '\n';
    ...
    for (const std::string& xml : *xml_events) {
        ...
        const auto parsed_event = panopticon::officer::query::SysmonParser::parse_xml(
            xml, error_message);
        if (!parsed_event) {
            std::cerr << "Officer Query parsing failed: " << error_message << '\n';
    ...
    if (options->output_path) {
        ...
        if (!output_file) {
            std::cerr << "Officer Query could not open output file: " << *options->output_path << '\n';
    ...
    for (const std::string& line : output_lines) {
        ...
        if (options->output_path) {
            output_file << line << '\n';
            if (!output_file) {
                std::cerr << "Officer Query could not write the complete output file.\n";
                return static_cast<int>(ExitCode::output_failure);
            }
        }
    }
